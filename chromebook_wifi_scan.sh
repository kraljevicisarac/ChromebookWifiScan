...
# set the name of our wirless adapter (run the 'iwconfig' command yourself and get the adapter name, which wil be somethng like 'wlan0' or 'wlx687f74807e6f')
WLAN_NIC="wlan0"

# set our path to 'iw' - enter "which iw" on the CLI if not sure usually "/sbin/iw" or "/usr/sbin/iw"
IW='/usr/sbin/iw'

# The script runs in a continous loop - set the pause time between each run (in secs) here (added
